    ...
    class PicSign {
        ...
        dominit() {
            ...
            var prompt = function (success, msg) {
                layer.prompt({ title: '输入姓名与信息(专业) 使用连字符“-”隔开', formType: 2, width: $this.Options.inputwidth, height: $this.Options.inputheight, value: msg }, function (text, index) {
                    layer.close(index);
                    if (text.length > 0) {
        ...
        dataset_one(signdata, istrigger) {
            ...
            if (signdata) {
                if (signdata.signid) {
                    ...
                    var copy = { left: data.left, top: data.top, msg: data.msg, title: data.title};
                    if (data) {
                        if (signdata.left) {
